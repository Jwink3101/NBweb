...
class JSONSession(BaseSession):
    '''Class which stores session information in the file-system w/ JSON.

    :param session_dir: Directory that session information is stored in.
            (default: ``'/tmp'``).
    '''
    def __init__(self, session_dir='/tmp', *args, **kwargs):
        super(JSONSession, self).__init__(*args, **kwargs)
        self.session_dir = session_dir
    ...
    def save(self, data):
        ...
        try:
            os.makedirs(os.path.split(tmpName)[0])
        except OSError:
            pass
        
        with open(tmpName, 'w') as fp:
            self.session = json.dump(data, fp)
        os.rename(tmpName, fileName)
